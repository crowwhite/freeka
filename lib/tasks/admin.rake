# TODO: Should create admin in a seed. No need for user input.
# Was asked by akhil sir to do by rake task
namespace :admin do
  # TODO: Why TODO??
  # It was generated by scaffold, I don't think I need it. Commenting for now
  desc "Create admin"
  task create: :environment do
    a = Admin.create(name: 'admin', email: 'mail2freeka@gmail.com', address: 'patel nagar', about_me: '..', contact_no: '5555999999', password: 'jjjjjjjj', slug: 'admin-1', confirmed_at: Time.current)
    is_saved = a.save
    p is_saved
    if is_saved
      puts 'Admin created with '
      puts a.email
      puts a.password
    end
  end

end
